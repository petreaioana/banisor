(() => {
  ...
  const missions = [
    ...
    { id: 'coins', icon: '🪙', category: 'Calcul', title: 'Casa de monede', description: 'Formează suma cerută folosind monede potrivite.', coach: 'Alege monedele și verifică suma când ești gata.' },
  ...
  function initCoins(root) {
    ...
    function render() {
      ...
      root.append(choices);
      const feedback = make('div', `feedback ${tone}`, message); feedback.setAttribute('aria-live', 'polite'); root.append(feedback);
      ...
      setCoach(getMission('coins').coach, round / targets.length, `Runda ${round + 1} din ${targets.length}`);
    }
  }
  ...
  function initNumberPath(root) {
    ...
    function choose(button, answer, options, card) { if (answered) return; answered = true; const good = answer === gates[index].correct; if (good) score += 1; options.querySelectorAll('button').forEach(item => { item.disabled = true; }); button.classList.add(good ? 'correct' : 'wrong'); const feedback = $('.feedback', card); feedback.className = `feedback ${good ? 'good' : 'warn'}`; feedback.textContent = good ? 'Corect. Poarta se deschide.' : 'Nu se potrivește. Recitește operația și verifică semnul.'; const next = make('button', 'primary-button', index === gates.length - 1 ? 'Vezi rezultatul' : 'Deschide poarta'); next.type = 'button'; next.style.marginTop = '13px'; next.addEventListener('click', () => { index += 1; answered = false; if (index >= gates.length) finishGame((score / gates.length) * 100, `${score} porți deschise corect din ${gates.length}.`); else render(); }); card.append(next); }
  }
  ...
  function initColorLab(root) {
    ...
    function choose(button, value, options, side, round) { if (answered) return; answered = true; const good = value === round.result; if (good) score += 1; options.querySelectorAll('button').forEach(item => { item.disabled = true; }); button.classList.add(good ? 'correct' : 'wrong'); const feedback = make('div', `feedback ${good ? 'good' : 'warn'}`, good ? `Corect. Rezultatul este ${round.result}.` : 'Nu se potrivește. Privește cele două culori și reține combinația pentru următoarea rundă.'); side.append(feedback); const next = make('button', 'primary-button', index === rounds.length - 1 ? 'Vezi rezultatul' : 'Următoarea combinație'); next.type = 'button'; next.style.marginTop = '14px'; next.addEventListener('click', () => { index += 1; answered = false; if (index >= rounds.length) finishGame((score / rounds.length) * 100, `${score} combinații potrivite din ${rounds.length}.`); else render(); }); side.append(next); }
  }
